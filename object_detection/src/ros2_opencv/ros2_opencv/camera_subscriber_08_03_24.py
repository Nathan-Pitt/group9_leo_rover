    ...
    if x_coordinate == -1:
        ...
        if len(contours) > 0:
            ...
            if depth[v][u] == 0:
                ...
                distance_to_block[index] = depth[v][u]
                object_close = 0
                index = (index + 1) % 5
            ...
            x_temp = np.mean(distance_to_block) * ((u - intrinsics.cx) / intrinsics.fx)
            y_temp = np.mean(distance_to_block) * ((v - intrinsics.cy) / intrinsics.fy)
            z_temp = np.mean(distance_to_block)
...
class ImageSubscriber(Node):
    ...
    def image_depth_info_callback(self, camera_info: CameraInfo):
        try:
            if self.intrinsics:
                return
            self.intrinsics = CameraIntrinsics(cx=camera_info.k[2],
                                               cy=camera_info.k[5],
                                               fx=camera_info.k[0],
                                               fy=camera_info.k[4])
